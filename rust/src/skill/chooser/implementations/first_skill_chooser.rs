...
impl FirstSkillChooser {
    ...
    fn choose(&mut self, skill_pool: Gd<SkillContainerModule>, skill_resource: Gd<SkillResourceModule>, target: Gd<BattleEntity>) {
        ...
        self.get_skill_chooser().unwrap().signals().skill_chosen().emit(name, &implementation, &skill_resource);
    }
}