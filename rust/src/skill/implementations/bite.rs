...
impl SkillImplementation for BiteSkillImplementation {
    fn cast(&mut self, mut user: Gd<BattleEntity>, targets: &Array<Gd<BattleEntity>>) {
        godot_print!("biteada de manual. nº targets: {}", targets.len());
        let node = self.base_mut().clone().upcast::<Node>();
        let animation_factory = get_skill_animation_factory_node_from_tree(&node);
        let mut animation = animation_factory
            .bind()
            .get_animation(SkillAnimationName::Bite);
        ...
        animation
            .bind_mut()
            .adapt_facing_direction_to_target(&user, &target);
